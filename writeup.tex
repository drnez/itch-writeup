...
\title{A lightweight package manager for Linux From Scratch systems}
...
For example, pkgA may require pkgC with version $\geq 3.0$, and pkgB may conflict with pkgC versions $> 3.5$. This means that for pkgA, pkgB, and pkgC to be simultaneously installed, pkgC must have version $v$, where $3.0 \leq v < 3.5$.
...
Firstly, this in effect turns the user's LFS system into a version of the distribution providing the package manager, and goes against much of the LFS philosophy, which is about creating and maintaining your own system, with full control over all software installed. The sheer complexity of these package managers removes such control from the users, and while it does often produce a working system, the user has in effect just followed a very long path to installing that distribution.
